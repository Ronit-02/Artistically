    ...
    <div className="relative flex-1 max-w-xl">
      ...
      {focused && (local.trim() ? results.length > 0 : recent.length > 0) && (
        <div ref={dropRef} className="animate-scale-in absolute top-full left-0 right-0 mt-2 bg-white border border-gray-100 rounded-xl shadow-xl z-50 overflow-hidden">
          {!local.trim() && recent.map(s => <button type="button" key={s} onClick={() => { setLocal(s); setSearchQuery(s); setFocused(false); onSearch(s); }} className="w-full min-h-11 flex items-center gap-2.5 px-4 py-2.5 hover:bg-[#f5f5f5] text-left bg-transparent border-none cursor-pointer"><svg aria-hidden="true" className="w-3 h-3 text-gray-500" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path d="M12 8v4l3 3m6-3a9 9 0 1 1-18 0 9 9 0 0 1 18 0z"/></svg><span className="text-[13px] text-gray-500">{s}</span></button>)}
          {local.trim() && results.map((r, idx) => <button type="button" key={r.id} onClick={() => { saveRecent(local); setFocused(false); router.push(r.href); }} className={`w-full min-h-11 flex items-center gap-3 px-4 py-2.5 text-left bg-transparent border-none cursor-pointer transition-colors ${idx === selIdx ? "bg-[#f5f5f5]" : "hover:bg-[#f5f5f5]"}`}>{r.image ? <div className="w-8 h-8 rounded-lg overflow-hidden bg-[#f0f0f0] flex-shrink-0 relative"><Image src={r.image} alt="" fill className="object-cover" sizes="32px"/></div> : <div className="w-8 h-8 rounded-lg bg-[#f0f0f0] flex-shrink-0"/>}<div className="flex-1 min-w-0"><p className="text-[13px] font-medium text-[#111] truncate">{r.title}</p>{r.subtitle && <p className="text-[12px] text-gray-500 truncate">{r.subtitle}</p>}</div></button>)}
          {local.trim() && results.length === 0 && <div className="p-6 text-center text-[13px] text-gray-500">No results</div>}
        </div>
      )}
    </div>
  );
...
function NavbarContent() {
  const router = useRouter();
  const { searchQuery, setSearchQuery, isLoggedIn, logout } = useAppStore();
  const { data: currentUser } = useCurrentUser();
  const { data: cart = [] } = useCart();
  const { data: wishlist = [] } = useWishlist();
  const queryClient = useQueryClient();
  const hasSession = isLoggedIn || !!currentUser;
  ...
  return (
    <header className="sticky top-0 z-50 bg-white border-b border-gray-100">
      ...
      <div className="max-w-[1240px] mx-auto px-6 sm:px-10 flex items-center gap-6 h-[60px]">
        ...
        <div className="hidden sm:flex flex-1"><SearchBar key={searchQuery} onSearch={doSearch}/></div>
        ...
        <nav aria-label="Primary navigation" className="hidden md:flex items-center gap-6">
          ...
          {canAccessArtistPortal && <Link href="/artist-portal" className="text-[13px] text-accent-600 hover:text-accent-700 transition-colors font-medium">Sell</Link>}
        </nav>
        ...
        <div className="flex items-center gap-0.5 flex-shrink-0 ml-auto sm:ml-0">
          ...
          <Link href="/wishlist" aria-label="Wishlist" className="relative p-3 rounded-lg text-gray-500 hover:text-accent-600 transition-colors">
            <svg aria-hidden="true" className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>
            {wishlist.length > 0 && <span className="absolute top-1.5 right-1.5 w-2 h-2 rounded-full bg-accent-500"/>}
          </Link>

          {/* Cart */}
          <Link href="/cart" aria-label={`Cart${cartCount > 0 ? `, ${cartCount} items` : ""}`} className="relative p-3 rounded-lg text-gray-500 hover:text-accent-600 transition-colors">
            <svg aria-hidden="true" className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24"><path d="M6 2 3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 0 1-8 0"/></svg>
            {cartCount > 0 && <span className="absolute top-0.5 right-0.5 bg-accent-600 text-white text-[12px] rounded-full min-w-5 h-5 px-1 flex items-center justify-center font-semibold">{cartCount}</span>}
          </Link>

          {/* Mobile search */}
          <button type="button" onClick={() => setMobileSearch(!mobileSearch)} aria-label={mobileSearch ? "Close search" : "Open search"} aria-expanded={mobileSearch} className="sm:hidden p-3 rounded-lg text-gray-500 hover:text-accent-600 bg-transparent border-none cursor-pointer transition-colors">
            <svg aria-hidden="true" className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
          </button>
        </div>
      </div>

      {mobileSearch && <div className="sm:hidden px-6 pb-3"><SearchBar key={searchQuery} onSearch={q => { doSearch(q); setMobileSearch(false); }}/></div>}

      {/* Category nav — spacious */}
      <div className="border-t border-gray-50">
        <nav aria-label="Artwork categories" className="max-w-[1240px] mx-auto px-6 sm:px-10 py-2.5 overflow-x-auto">
           <ul className="flex items-center justify-start sm:justify-center gap-6 sm:gap-8 list-none m-0 p-0 whitespace-nowrap">
            {CATEGORY_LABELS.map(cat => (
              <li key={cat}><button type="button" onClick={() => catClick(cat)} className="inline-flex min-h-11 items-center text-[13px] text-gray-500 hover:text-accent-600 transition-colors bg-transparent border-none cursor-pointer px-1 font-medium">{cat}</button></li>
            ))}
          </ul>
        </nav>
      </div>
